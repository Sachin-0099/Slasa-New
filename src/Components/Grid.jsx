  ...
  if (specialLayout === "bulkDiscounts") {
    return (
      <div className="!bg-white p-4 shadow rounded-lg flex flex-col items-center justify-center">
        <img src={items[0].image} alt={items[0].alt} className="w-60 mt-2 cursor-pointer rounded-lg" onClick={() => handleClick(items[0])} />
    ...
    return (
      <div className="bg-white p-4   shadow rounded-lg flex justify-between items-center col-span-4">
        ...
        {items.map((item, index) => (
          <img key={index} src={item.image} alt={item.alt} className="w-60 h-48 m-2 object-contain cursor-pointer" onClick={() => handleClick(item)} />
        ))}
      </div>
    );
  }
      ...
      <div className="grid grid-cols-2 gap-2 mt-2">
        {items.map((item, index) => (
          <div key={index} className="flex flex-col items-center cursor-pointer" onClick={() => handleClick(item)}>
            <img src={item.image} alt={item.alt} className="w-40 h-30 object-cover" />
            {item.caption && <p className="text-sm text-center">{item.caption}</p>}
          </div>
        ))}
      </div>
      {linkText && <a href={linkHref} className="text-[#3087d1] text-lg text-center relative block mt-2">{linkText}</a>}
    </div>
  );
};
